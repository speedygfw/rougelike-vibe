# 📖 Game Guide & Manual

Welcome to the official guide for **Mein Roguelike**. This manual provides detailed information about gameplay mechanics, controls, and the creatures you will encounter in the dungeon.

## 🎮 Controls

| Action | Key / Input | Description |
| :--- | :--- | :--- |
| **Move** | `Arrow Keys` | Move your character North, South, East, or West. |
| **Attack** | `Arrow Keys` | Move into an enemy to attack them with your equipped weapon. |
| **Pickup Item** | `g` | Pick up an item standing on your current tile. |
| **Inventory** | `i` | Toggle the inventory screen to view and use items. |
| **Interact** | `Space` / `Enter` | Interact with stairs to descend to the next level. |
| **Cast Spell** | `1` - `9` | Cast a spell from your spellbook (if you have mana). |
| **Restart** | `r` | Restart the game after death or victory. |
| **Mouse** | `Click` | Select class at start, click inventory items to use them. |

## ⚔️ Character Classes

Choose your path wisely at the start of the game:

*   **Warrior**: High HP and Strength. Starts with a sword. Best for melee combat.
*   **Mage**: High Mana and Intelligence. Starts with spells. Best for ranged magic attacks.
*   **Rogue**: High Agility and Critical Chance. Starts with a dagger. Best for stealth and precision.

## 🐉 Enemy Bestiary

Beware of the monsters lurking in the dark. Different factions may fight each other!

| Symbol | Name | HP | Atk | Def | Faction | Special Notes |
| :---: | :--- | :--- | :--- | :--- | :--- | :--- |
| `r` | **Rat** | 8 | 2 | 0 | Beast | Weakest enemy. Often found in swarms. |
| `b` | **Bat** | 10 | 2 | 0 | Beast | Erratic movement. Hard to pin down. |
| `🕷️` | **Spider** | 12 | 4 | 1 | Beast | Average threat. |
| `k` | **Kobold** | 15 | 3 | 1 | Monster | Weak but cunning. |
| `g` | **Goblin** | 20 | 5 | 1 | Monster | Common dungeon dweller. |
| `💀` | **Skeleton** | 12 | 4 | 1 | Undead | Animated bones. |
| `S` | **Shaman** | 15 | 4 | 1 | Monster | **Ranged Magic**. Flees if you get too close. |
| `Z` | **Zombie** | 25 | 6 | 2 | Undead | Tough and relentless. |
| `👻` | **Ghost** | 15 | 5 | 5 | Undead | **High Defense**. Ethereal form makes them hard to hit. |
| `O` | **Orc** | 40 | 8 | 3 | Monster | Strong and durable. A serious threat. |
| `👹` | **Ogre** | 60 | 12 | 4 | Monster | Massive damage. Approach with caution. |
| `🐉` | **Dragon** | 150 | 20 | 8 | Dragon | **BOSS**. Uses **Firebreath** (AOE). The ultimate challenge on Level 10. |

### Factions
Enemies belong to factions. Hostile factions will attack each other if they cross paths:
*   **Player**: You.
*   **Monster**: Goblins, Orcs, Ogres, Kobolds, Shamans.
*   **Undead**: Skeletons, Zombies, Ghosts.
*   **Beast**: Rats, Bats, Spiders.
*   **Dragon**: The Dragon (hostile to all).

## 🎒 Items & Equipment

*   **Potions**: Restore Health (`Red`) or Mana (`Blue`).
*   **Scrolls**:
    *   *Fireball*: Deals damage to all enemies in a radius.
    *   *Teleport*: Instantly moves you to a random safe location.
*   **Weapons**: Increase your Attack damage.
*   **Armor**: Increase your Defense, reducing damage taken.
*   **Amulet of Yendor**: The goal of the game. Found on Level 10.

## 📜 Tips for Survival

1.  **Rest to Heal**: You regenerate Mana over time. Use healing spells if you are a Mage.
2.  **Use Chokepoints**: Fight enemies one by one in doorways.
3.  **Watch Factions**: If you see a Skeleton fighting a Goblin, let them weaken each other before engaging!
4.  **Explore**: Find all items on a level before descending, but don't get surrounded.
